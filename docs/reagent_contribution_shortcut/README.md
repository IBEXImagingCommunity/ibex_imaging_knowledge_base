...
First, download the full Knowledge-Base [zip file](https://github.com/IBEXImagingCommunity/ibex_imaging_knowledge_base/archive/refs/heads/main.zip) and navigate to the `docs/reagent_contribution_shortcut` directory. This directory contains an example contribution and you will need to provide your data in a similar fashion.
...
2. `Publications` column - If there are one or more publications associated with a reagent validation, list each one in a markdown file and reference them in this column (file names are separated by a semicolon). If there are no publications, leave the column entry empty.
